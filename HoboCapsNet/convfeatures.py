...
def load_next_batch(sess, io):
    for batch_idx in range(0, len(files), batch_size):
        ...
        batch = np.array(
            map(lambda x: load_image(sess, io, x), batch))
        batch = batch.reshape((batch_size, 299, 299, 3))
        yield batch


def forward_pass(io, coco_img_path, flickr_img_path):
    global output_layer, files
    files = sorted(np.concatenate((np.array([coco_img_path+p for p in os.listdir(coco_img_path)]), np.array([flickr_img_path+p for p in os.listdir(flickr_img_path)])), axis=0))
...
def get_arguments():
    ...
    parser.add_argument(
        "--coco_data_path",
        type=str,
        help="A valid path to MSCCOCO",
        required=True)
    parser.add_argument(
        "--flickr_data_path",
        type=str,
        help="A valid path to flickr30k",
        required=True)
    parser.add_argument(
        "--inception_path",
        type=str,
...
if __name__ == "__main__":
    ...
    forward_pass(io, args.coco_data_path, args.flickr_data_path)
    print "done"
