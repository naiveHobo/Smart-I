...
parser.add_argument(
    "--coco_caption_path",
    type=str,
    help="A valid path to COCO: captions_val2017.json")
parser.add_argument(
    "--flickr_caption_path",
    type=str,
    help="flickr30k caption file: results_20130124.token")
parser.add_argument(
    "--feature_path",
    type=str,
...
if config.mode == "train":
    vocab, wtoidx, training_data = generate_captions(
        config.word_threshold, config.max_len, args.coco_caption_path, args.flickr_caption_path, args.feature_path,
        config.data_is_coco)
