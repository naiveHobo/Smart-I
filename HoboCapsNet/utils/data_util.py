...
def generate_captions(
        wt=2,
        ml=20,
        coco_cap_path='Dataset/coco-images/results_20130124.token',#default set to flickr30k captions
        flickr_cap_path='Dataset/flickr-images/results_20130124.token',
    ...
    if data_is_coco:
        ...
        coco_cap_path = prepare_coco_captions(coco_cap_path)
        # Load the COCO captions data
        with open(coco_cap_path, 'r') as f:
            data = f.readlines()
        ...
        with open(flickr_cap_path, 'r') as f:
            data = f.readlines()
        filenames = [caps.split('\t')[0].split('#')[0] for caps in data]
    ...
    idx = np.random.permutation(158900)
    df = df.iloc[idx]
